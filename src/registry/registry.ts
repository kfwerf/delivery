...
export default class GrpcTypeRegistry {
  ...
  registerMessage(type: GrpcMessage): GrpcTypeRegistry {
    this.registryMessage.set(type.getPath(), type);
    return this;
  }

  registerService(type: GrpcService): GrpcTypeRegistry {
  ...
  listMessagesByDependency(): GrpcMessage[] {
    return Array.from(this.registryMessage.values()).sort(
      (a, b) => a.getDependencies().length - b.getDependencies().length,
    );
  }
}
