...
import {
    ...
    getMessagePropertyObjectIndexes, getOneOfValue
...
export default class GrpcMessage {
    ...
    constructor(response: GrpCurlResponse) {
      ...
      const oneOfProperties = oneOfIndexes.map(({begin, end}) => {
    ...
    public isFaulty(): boolean {
        const count = this.properties
            .map(property => property.getOrdinal())
            .reduce((a, b) => a + b, 0);
        ...
        const newExample = this.properties
            .filter((property, idx) => {
                ...
                if (property.getOneOfKey().length) {
                    const oneOfKeysList = this.properties
                        .map((prop) => prop.getOneOfKey());
