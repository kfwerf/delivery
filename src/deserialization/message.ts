...
export const getMessagePathFromRequest = (request: string): string =>
  matchSafe(request, /describe [\w.]+/gm)
    .join('')
    .slice(9);

export const getMessagePath = (response: GrpCurlResponse): string =>
  matchSafe(response.getData(), /.+ is a message:/gm)
    .join('')
    .replace(/ is a message:/, '');
...
export const getMessageProperties = (response: GrpCurlResponse): { type: string; key: string }[] =>
  matchSafe(response.getData(), /[a-zA-Z0-9._]+ [a-zA-Z0-9._]+ = [0-9]/gi).map((prop) => {
...
export const getMessagePropertyLinesFiltered = (
  lines: string[],
  objectIndexes: { end: number; begin: number }[],
  onlyObjects = false,
): string[] => {
  const isObjectRange = (idx: number): boolean =>
    objectIndexes.filter((range) => idx >= range.begin && idx <= range.end).length > 0;
  return lines.filter((line, idx) => (onlyObjects ? isObjectRange(idx) : !isObjectRange(idx)));
};
