...
test('extracts requests from a list of services', () => {
  ...
  const list = [new GrpcService(serviceOne), new GrpcService(serviceTwo), new GrpcService(serviceThree)];
...
test('extracts type names from a input', () => {
  const types = getTypeNames(
    'map<.com.delivery.v1.messages.EntityType, .google.protobuf.FieldMask> fieldmask_map = 2;',
  );
  expect(types).toStrictEqual(['.com.delivery.v1.messages.EntityType', '.google.protobuf.FieldMask']);
});
