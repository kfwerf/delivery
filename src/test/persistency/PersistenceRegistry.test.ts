...
test('should replace old url entry', () => {
  ...
  const expected: UrlEntry[] = [{ url: 'bar', date: 1624220730114 }, { url: 'baz', date: 1624220730514 }, newEntry];
...
test('should remove earliest url entry when limit is reached', () => {
  ...
  const expected: UrlEntry[] = [{ url: 'baz', date: 1624220730514 }, newEntry, { url: 'foo', date: 1624220730214 }];
  expect(urls).toHaveLength(3);
  expect(urls).toStrictEqual(expected);
});
